...
def _transform_boxes(boxes, boxID=1):
    """ Transform roidb according to boxID
    ...
    # transform rois according to boxID
    if boxID == 1:
        rois = rois_ori
    if boxID == 2:
        rois = rois_ori
        rois[:,2] = (rois[:,0] + rois[:,2])/2
    if boxID == 3:
        rois = rois_ori
        rois[:,0] = (rois[:,0] + rois[:,2])/2
    if boxID == 4:
        rois = rois_ori
        rois[:,3] = (rois[:,1] + rois[:,3])/2
    if boxID == 5:
        rois = rois_ori
    ...
    if boxID == 6:
        ...
        ws = rois[:,2] - rois[:,0]
        hs = rois[:,3] - rois[:,1]
        rois[:,0] = rois[:,0] + (0.5-r/2)*ws
        rois[:,2] = rois[:,2] - (0.5-r/2)*ws
        rois[:,1] = rois[:,1] + (0.5-r/2)*hs
        rois[:,3] = rois[:,3] - (0.5-r/2)*hs
    if boxID == 7:
        r = 0.8
        rois = rois_ori
        ws = rois[:,2] - rois[:,0]
...
def test_net(net, imdb,startIdx=0,endIdx=-1,boxes_num_per_batch=400,saveMat=False, aboxesname='aboxes',svm=False):
    ...
    output_dir = '/home/zhouhui/fast-rcnn-ilsvrc-test/data/results'
    output_dir = os.path.join(output_dir, net.name)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_dir = os.path.join(output_dir, imdb.name)
    if not os.path.exists(output_dir):
	os.makedirs(output_dir)
    # timers
    ...
    for i in xrange(num_images):
        mat_file1='%s/%d.mat'%(output_dir,i)
	if i < startIdx or i>=endIdx or os.path.isfile(mat_file1):
            continue
    ...
    mat_file = '/data2/zwang/py-faster-rcnn-master/'+ aboxesname + '.mat'
    sio.savemat(mat_file,{'aboxes':all_boxes})
