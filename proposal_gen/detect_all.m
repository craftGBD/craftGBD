function parsed_all = detect_all( list, param, caffe_solver, scale, thres, display, cutedge )
    ...
    for i = 1 : test_time
        ...
        for id = 1 : length(param.gpu_id)
            if ischar(list{i})
                img_t{id} = imread(fullfile(param.dataset_root, list{now_id(id)}));
        ...
        for s = scale
            ...
            if target_now > param.max_img
                for j = 1 : length(param.gpu_id)
                    [ img{j}, scale_now(j) ] = scale_img_to_target( img_t{j}, target_now ); % scale = now / pre
                end
                parsed_t = detect_large_img(img, caffe_solver, param, thres);
            else
                for j = 1 : length(param.gpu_id)
